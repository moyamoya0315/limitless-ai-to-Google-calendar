...
def extract_events_with_claude(lifelog):
    ...
    prompt = f"""あなたは会話ログからカレンダー登録の意図を抽出するアシスタントです。
...
## タスク
上記の会話ログを分析し、話者がカレンダーへの予定登録を意図している箇所を抽出してください。

### 抽出すべき意図の例
- 「明日15時から渋谷でAさんと打ち合わせ登録して」
- 「来週月曜の10時に歯医者の予定入れて」
- 「金曜日のランチをカレンダーに入れておいて」
- 「3月10日 14時から会議室Bでミーティング」
...
意図が見つからない場合は空配列 `{{"events": []}}` を返してください。
